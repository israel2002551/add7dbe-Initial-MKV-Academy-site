...
(function () {
  ...
  function videoMarkup(video, index) {
    ...
    if (!src) {
      return `
        <article class="mkv-card overflow-hidden" data-reveal data-reveal-delay="${index * 80}">
          ...
          <div class="p-5">
            <p class="font-technical text-xs uppercase tracking-widest text-brand-700">${video.label || `Video ${index + 1}`}</p>
            <h3 class="mt-2 font-bold text-slate-900">${video.title}</h3>
